        ...
        <div class="container position-relative" style="z-index:2">
            <div class="row align-items-center min-vh-75 py-5">
                <div class="col-lg-6 text-white">
                    <span class="badge bg-primary bg-opacity-25 text-primary-light px-3 py-2 mb-3 rounded-pill">📚
                        Plateforme de Gestion de Bibliothèque</span>
                    ...
                    <p class="fs-5 text-white-50 mb-4 pe-lg-5">
                        Gérez vos emprunts, découvrez de nouveaux livres et accédez à un catalogue riche et diversifié — le
                        tout depuis une plateforme moderne et intuitive.
                    </p>
                    <div class="d-flex gap-3 flex-wrap">
                        @auth
                            <a href="{{ route('dashboard') }}"
                                class="btn btn-primary btn-lg px-5 rounded-pill shadow-lg btn-glow">
    ...
    <section class="py-5 bg-light">
        <div class="container">
            <div class="text-center mb-5">
                ...
                <p class="text-muted col-md-6 mx-auto">Découvrez les ouvrages les plus empruntés par notre communauté de
                    lecteurs.</p>
            </div>
            <div class="row g-4">
                @foreach($livresPopulaires as $livre)
                    <div class="col-md-4 col-lg-2">
                        <div class="card h-100 border-0 shadow-sm book-card rounded-4 overflow-hidden">
                            {{-- IMAGE --}}
                            <div class="book-cover-wrapper bg-light position-relative text-center d-flex align-items-center justify-content-center overflow-hidden"
                                style="height: 250px;">
                                @if($livre->couverture)
                                    <img src="{{ \Illuminate\Support\Str::startsWith($livre->couverture, ['http://', 'https://']) ? $livre->couverture : asset('storage/' . $livre->couverture) }}"
                                        class="w-100 h-100 object-fit-cover transition-img" alt="{{ $livre->titre }}"
                                        onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
                                    <div class="w-100 h-100 align-items-center justify-content-center transition-img"
                                        style="display:none; background: linear-gradient(135deg, {{ ['#1e3a8a', '#7c3aed', '#059669', '#dc2626', '#d97706', '#0891b2'][$livre->id % 6] }}, {{ ['#3b82f6', '#a78bfa', '#34d399', '#f87171', '#fbbf24', '#22d3ee'][$livre->id % 6] }});">
                                ...
                                @else
                                    <div class="w-100 h-100 d-flex align-items-center justify-content-center transition-img"
                                        style="background: linear-gradient(135deg, {{ ['#1e3a8a', '#7c3aed', '#059669', '#dc2626', '#d97706', '#0891b2'][$livre->id % 6] }}, {{ ['#3b82f6', '#a78bfa', '#34d399', '#f87171', '#fbbf24', '#22d3ee'][$livre->id % 6] }});">
                                        <span style="font-size: 64px; filter: drop-shadow(0 4px 8px rgba(0,0,0,0.3));">📖</span>
                                    </div>
                                @endif
                                ...
                                <div class="book-overlay d-flex align-items-center justify-content-center">
                                    <a href="{{ route('livres.show', $livre->id) }}"
                                        class="btn btn-light btn-sm fw-bold rounded-pill px-3 shadow">
                                        <i class="bi bi-eye"></i> Voir Détails
                                    </a>
                                </div>

                                <div class="position-absolute top-0 end-0 m-2 z-2">
                                    <span class="badge bg-warning text-dark rounded-pill shadow-sm px-2 py-1">
                                        🔥 {{ $livre->emprunts_count }}
                                    </span>
                                </div>
                            </div>
                            ...
                            <div class="card-body d-flex flex-column p-3">
                                <div class="mb-auto text-center">
                                    <h6 class="fw-bold text-dark mb-1 text-truncate" title="{{ $livre->titre }}">
                                        {{ $livre->titre }}
                                    </h6>
                                    <p class="text-muted small mb-2">{{ $livre->auteur }}</p>
                                    <span
                                        class="badge bg-light text-dark border small">{{ $livre->theme->intitule ?? '' }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="container">
            ...
            <div class="row g-3 justify-content-center">
                @foreach($themes as $theme)
                    <div class="col-auto">
                        <a href="{{ route('livres.index', ['themeSel' => $theme->id]) }}"
                            class="theme-pill d-flex align-items-center gap-2 text-decoration-none">
    ...
    <section class="py-5">
        <div class="container">
            <div class="cta-card text-center text-white p-5 rounded-4">
                ...
                <p class="fs-5 text-white-50 mb-4">Rejoignez notre communauté de lecteurs et accédez à un catalogue riche et
                    diversifié.</p>
